...
- ✅ B1:react-native-godot example 三平台跑通,双向 signal 可用 — 详见 [_B1_REPORT.md](doc/cute_pixel_spec/_B1_REPORT.md)
...
| Android Studio + SDK + AVD | 最新 | AVD 必须 arm64-v8a([conventions §14c](doc/cute_pixel_spec/conventions.md#14c-android-ndk--镜像准备)) |
| Godot 编辑器 | **4.5.x 钉死** | 不能用 4.6+([ADR-002](doc/cute_pixel_spec/decisions/ADR-002-godot-as-pixel-engine-via-react-native-godot.md)) |
| OpenJDK | 21 | Android build |
| Ruby + bundler | 2.6+ + 2.4.1 user-install | iOS CocoaPods([conventions §14d](doc/cute_pixel_spec/conventions.md#14d-ios-ruby--bundler)) |
...
├── scripts/              # Godot 导出脚本等
├── doc/cute_pixel_spec/  # 架构 / 约定 / ADR(本仓的"为什么这样做")
├── ios/                  # 自动生成的 RN iOS native 工程
└── android/              # 自动生成的 RN Android native 工程
```

业务开发新模块的具体规则见 [architecture.md](doc/cute_pixel_spec/architecture.md):4 条铁律 + Module-First Flat + 轻 DDD 视角。
...
| 项 | 选择 |
|---|---|
| App 框架 | React Native 0.81 Bare + Expo modules autolinking([ADR-001](doc/cute_pixel_spec/decisions/ADR-001-react-native-as-app-framework.md), [ADR-004](doc/cute_pixel_spec/decisions/ADR-004-rn-bare-workflow.md)) |
| 像素引擎 | Godot 4.5.x(编辑器版本钉死)+ LibGodot 4.5.1.migeran.2(via react-native-godot 1.0.1)([ADR-002](doc/cute_pixel_spec/decisions/ADR-002-godot-as-pixel-engine-via-react-native-godot.md)) |
| RN ↔ Godot 通信 | Single typed message bus + RN/GD 状态权属铁律 + 仓库根 `proto/`([ADR-007](doc/cute_pixel_spec/decisions/ADR-007-rn-godot-communication-contract.md)) |
| 状态管理 | Zustand([ADR-003](doc/cute_pixel_spec/decisions/ADR-003-state-management-zustand.md)) |
| 包管理 | yarn 4 Berry(via corepack) |
| Lint | Biome |
| 测试 | Jest + RNTL + Detox |
| 像素 asset 编排 | Godot 4.5 编辑器([ADR-005](doc/cute_pixel_spec/decisions/ADR-005-godot-as-asset-editor.md)) |
| 业务流水线 | Spec-driven(PRD → TechPack → module-gen → test-gen → review)([ADR-006](doc/cute_pixel_spec/decisions/ADR-006-spec-driven-with-strong-gates.md)) |

## 文档地图

- [doc/cute_pixel_spec/README.md](doc/cute_pixel_spec/README.md) — 文档草稿区索引(ADR 状态速查)
- [doc/cute_pixel_spec/architecture.md](doc/cute_pixel_spec/architecture.md) — 模块边界 / 4 条铁律 / Module-First Flat 命名
- [doc/cute_pixel_spec/conventions.md](doc/cute_pixel_spec/conventions.md) — 14 条编码标准(P0 + P1)
- [doc/cute_pixel_spec/pixel-foundation.md](doc/cute_pixel_spec/pixel-foundation.md) — RN ↔ Godot 通信契约 / GodotProvider + PixelView Portal
- [doc/cute_pixel_spec/decisions/](doc/cute_pixel_spec/decisions/) — 6 个 ADR
- [doc/cute_pixel_spec/_B1_REPORT.md](doc/cute_pixel_spec/_B1_REPORT.md) — B1 集成验证报告
